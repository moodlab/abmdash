...
| `Failed to parse REDCap response` or HTTP `401`/`403` | The token is wrong, was rotated, or belongs to a different REDCap project | Re-copy the token from the REDCap project page → *API* tab; confirm it is the token for the **screening** project that owns reports 14081 and 13387 |
...
score[is.na(score)] <- 0            # or keep and drop: score[!is.na(score)]
