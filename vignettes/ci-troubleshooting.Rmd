...
| Workflow green, but the dashboard on the repo shows stale or blank numbers | The last successful run produced no visual change (dashboard re-rendered identically), the cron ran while a data source was unreachable, or you are looking at an old cached page | Check the artifacts: every run uploads `docs/` as an artifact (`dashboard-<run number>`, kept 30 days) with the exact HTML that was committed. Compare artifact timestamps with the data you expected |
