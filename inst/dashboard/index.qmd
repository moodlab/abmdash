...
trad_compliance <- tryCatch({
  ...
  if (service_account_json == "" || is.na(service_account_json)) {
    data.frame(
      Message = "ABS credentials not configured"
    )
  } else {
    # Login and download data
    diagnostics$stage <- "logging in"
  ...
  diag_info <- if (exists("diagnostics") && is.environment(diagnostics)) {
    parts <- c(
      sprintf("Stage: %s", diagnostics$stage %||% "unknown"),
      sprintf("Raw rows: %s", diagnostics$raw_rows %||% "N/A"),
      sprintf("Columns: %s", diagnostics$raw_cols %||% "N/A"),
      sprintf("Has start_time: %s", diagnostics$has_start_time %||% "N/A"),
      sprintf("start_time class: %s", diagnostics$start_time_class %||% "N/A"),
      sprintf("start_time length: %s", diagnostics$start_time_length %||% "N/A"),
      sprintf("start_time sample: %s", diagnostics$start_time_sample %||% "N/A"),
      sprintf("Filtered rows: %s", diagnostics$filtered_rows %||% "N/A"),
      sprintf("Session 1 rows: %s", diagnostics$session1_rows %||% "N/A"),
      sprintf("First sessions: %s", diagnostics$first_sessions_rows %||% "N/A"),
      sprintf("Active rows: %s", diagnostics$active_rows %||% "N/A"),
      sprintf("Session counts: %s", diagnostics$session_counts_rows %||% "N/A"),
      sprintf("IDs overlap: %s", diagnostics$ids_overlap %||% "N/A"),
      sprintf("Active IDs: %s", diagnostics$active_ids_sample %||% "N/A"),
      sprintf("Session IDs: %s", diagnostics$session_ids_sample %||% "N/A"),
      sprintf("Merged rows: %s", diagnostics$merged_rows %||% "N/A"),
      sprintf("NA count: %s", diagnostics$na_count %||% "N/A")
    )
  ...
  data.frame(
    Info = c("Error", "Diagnostics"),
    Value = c(
      substr(as.character(e$message), 1, 300),
      substr(diag_info, 1, 500)
    ),
    stringsAsFactors = FALSE
  ) |>
    tinytable::tt()
