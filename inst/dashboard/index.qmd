...
trad_compliance <- tryCatch({
  ...
  if (service_account_json == "" || is.na(service_account_json)) {
    ...
    message("DEBUG: Initial trad_data rows: ", nrow(trad_data))
    message("DEBUG: Columns: ", paste(names(trad_data), collapse = ", "))
    ...
    message("DEBUG: After filtering tests: ", nrow(trad_data), " rows")

    # Get first session date per participant
    session_1_data <- trad_data[trad_data$session == 1, ]
    message("DEBUG: Session 1 data rows: ", nrow(session_1_data))

    first_sessions <- aggregate(start_time ~ subject_id,
                               data = session_1_data,
                               FUN = min)
    message("DEBUG: first_sessions rows: ", nrow(first_sessions))
    ...
    if (nrow(active) > 0) {
      ...
      session_counts <- aggregate(session ~ subject_id,
                                 data = trad_data,
                                 FUN = length)
      message("DEBUG: session_counts rows: ", nrow(session_counts))
      message("DEBUG: session_counts columns: ", paste(names(session_counts), collapse = ", "))

      names(session_counts) <- c("id", "total_sessions")

      # Merge - use safer approach
      if (nrow(session_counts) > 0) {
        message("DEBUG: Before merge - active rows: ", nrow(active), ", session_counts rows: ", nrow(session_counts))
        active_merged <- merge(active, session_counts, by = "id", all.x = TRUE)
        message("DEBUG: After merge - rows: ", nrow(active_merged), ", columns: ", paste(names(active_merged), collapse = ", "))

        # Replace NA with 0
        if ("total_sessions" %in% names(active_merged)) {
          na_count <- sum(is.na(active_merged$total_sessions))
          message("DEBUG: NAs in total_sessions: ", na_count)
          active_merged$total_sessions[is.na(active_merged$total_sessions)] <- 0
        } else {
          message("DEBUG: total_sessions column missing, adding with 0s")
          active_merged$total_sessions <- 0
        }
        active <- active_merged
      } else {
        message("DEBUG: No session counts, adding total_sessions = 0")
  ...
  data.frame(
    Message = paste("Error loading Traditional ABM data:", substr(e$message, 1, 50))
  ) |>
    tinytable::tt()
})
