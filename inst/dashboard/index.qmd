    ...
    if (is.list(events) && "items" %in% names(events) && length(events$items) > 0) {
      ...
      if (length(filtered_events) > 0) {
        ...
        events_df <- data.frame(
          Time = character(n_events),
          Event = character(n_events),
          SortDate = as.POSIXct(rep(NA, n_events)),
          IsToday = logical(n_events),
          stringsAsFactors = FALSE
        )
        ...
        for (i in seq_along(filtered_events)) {
          ...
          # Extract time and date from start datetime - with guard clauses
          if (!is.null(event$start$dateTime)) {
            # Parse and format the datetime
            dt <- tryCatch({
              as.POSIXct(event$start$dateTime, format = "%Y-%m-%dT%H:%M:%S", tz = "UTC")
            }, error = function(e) {
              # Try alternative parsing if standard format fails
              as.POSIXct(gsub("Z$", "", event$start$dateTime), tz = "UTC")
            })

            # Guard clause: ensure valid datetime
            if (is.na(dt)) {
              events_df[i, ] <- list(
                Time = "Invalid DateTime",
                Event = event$summary %||% "No title",
                SortDate = as.POSIXct(Sys.time()),
                IsToday = FALSE
              )
              next
            }
            ...
            event_date <- tryCatch({
              as.Date(event$start$date)
            }, error = function(e) {
              as.Date(Sys.Date())
            })

            # Guard clause: ensure valid date
            if (is.na(event_date)) {
              events_df[i, ] <- list(
                Time = "Invalid Date",
                Event = event$summary %||% "No title",
            ...
            # For all-day events, set sort date to beginning of day
            sort_date <- as.POSIXct(paste(event_date, "00:00:00"), tz = "UTC")
          } else {
            # Guard clause: no valid start time
            events_df[i, ] <- list(
              Time = "Unknown",
              Event = event$summary %||% "No title",
              SortDate = as.POSIXct(Sys.time()),
          ...
          # Get event summary with guard clause
