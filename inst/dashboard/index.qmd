...
trad_compliance <- tryCatch({
  ...
  if (abs_username == "" || is.na(abs_username) || abs_password == "" || is.na(abs_password)) {
    ...
    login_result <- tryCatch({
      ...
      list(success = TRUE, session = session, error = NULL)
    }, error = function(e) {
      list(success = FALSE, session = NULL, error = as.character(e$message))
    })
    ...
    # Check if session has cookie preservation
    diagnostics$session_class <- class(session)[1]

    # Try to download CSV and capture response details
    csv_result <- tryCatch({
      ...
      # Capture the URL being requested
      req_url <- httr2::req_dry_run(req, quiet = TRUE, redact_headers = FALSE)$url
      diagnostics$csv_url <- req_url
      ...
      list(
        ...
        body_preview = substr(body, 1, 500),
        body = body
      )
    }, error = function(e) {
      list(status = "error", error = as.character(e$message))
    })

    diagnostics$csv_status <- csv_result$status
    diagnostics$csv_content_type <- csv_result$content_type %||% "unknown"
    diagnostics$csv_body_length <- csv_result$body_length %||% 0
    diagnostics$csv_body_preview <- csv_result$body_preview %||% "none"

    if (csv_result$status != 200) {
      stop("CSV download failed with status: ", csv_result$status)
    }

    # Parse the CSV
    trad_data <- utils::read.csv(text = csv_result$body, stringsAsFactors = FALSE)
  ...
  if (exists("diagnostics") && is.environment(diagnostics)) {
    ...
    if (!is.null(diagnostics$csv_status)) {
      diag_parts <- append(diag_parts, list(
        data.frame(Info = "CSV URL", Value = diagnostics$csv_url %||% "unknown", stringsAsFactors = FALSE),
        ...
        data.frame(Info = "HTML Title", Value = {
          body <- diagnostics$csv_body_preview %||% ""
          title_match <- regmatches(body, regexpr("<title>([^<]*)</title>", body, ignore.case = TRUE, perl = TRUE))
          if (length(title_match) > 0) gsub("</?title>", "", title_match[1], ignore.case = TRUE) else "N/A"
        }, stringsAsFactors = FALSE)
      ))
    }
