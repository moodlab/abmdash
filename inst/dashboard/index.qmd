...
trad_compliance <- tryCatch({
  ...
  if (abs_username == "" || is.na(abs_username) || abs_password == "" || is.na(abs_password)) {
    ...
    # DEFINITIVE TEST: Make request to httpbin to see what cookies are actually sent
    test_req <- session |>
      httr2::req_url("https://httpbin.org/headers") |>
      httr2::req_error(is_error = function(resp) FALSE) |>
      httr2::req_perform()

    if (httr2::resp_status(test_req) == 200) {
      test_body <- httr2::resp_body_json(test_req)
      diagnostics$httpbin_cookies_sent <- test_body$headers$Cookie %||% "NONE"
    } else {
      diagnostics$httpbin_cookies_sent <- "httpbin request failed"
    }
    ...
    if (!is.null(session_cookies) && length(session_cookies) > 0) {
      # Flatten httr2_headers to plain character vector
      cookie_values <- unlist(session_cookies, use.names = FALSE)
      # Extract just the cookie values (before the semicolon)
      cookie_values <- sapply(cookie_values, function(c) {
        strsplit(as.character(c), ";")[[1]][1]
      }, USE.NAMES = FALSE)
      cookie_string <- paste(cookie_values, collapse="; ")
      diagnostics$cookie_preview <- substr(cookie_string, 1, 200)

      # Make request using session's cookie file (don't override with manual header)
      req <- session |>
        httr2::req_url_path("/admin/test/download-csv-all") |>
        httr2::req_options(http_version = 0, verbose = TRUE)  # Verbose to see what's sent
  ...
  if (exists("diagnostics") && is.environment(diagnostics)) {
    ...
    if (!is.null(diagnostics$session_class)) {
      diag_parts <- append(diag_parts, list(
        ...
        data.frame(Info = "HTTPBin Cookies Sent", Value = substr(diagnostics$httpbin_cookies_sent %||% "unknown", 1, 300), stringsAsFactors = FALSE),
