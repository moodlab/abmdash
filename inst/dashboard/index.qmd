...
calendar_events <- tryCatch({
  ...
  if (service_account_json == "" || is.na(service_account_json)) {
    ...
    # Fetch events from both Research Team Schedule and Clinician Scheduling calendars
    events <- abmdash::get_combined_calendar_events(
      calendar_ids = c(
        "ff5b00ef944953433c805960d64adae47a456ee0d2515410cc648f65bafc2635@group.calendar.google.com",
        "5579dfb7124755be5221137e0f182c5ac8ea2d72001e9c23726fdaecbbccf242@group.calendar.google.com"
      ),
    ...
    if (is.list(events) && "items" %in% names(events) && length(events$items) > 0) {
      # Filter events to participant-related sessions only
      filtered_events <- Filter(function(event) {
        summary <- event$summary %||% ""
        grepl("UT Attention Bias Modification|Clinician Follow-Up Interview", summary, ignore.case = TRUE)
      }, events$items)
      ...
      if (length(filtered_events) > 0) {
        ...
        for (i in seq_along(filtered_events)) {
          ...
          # For clinician calendar events, extract clinician name from title
          # e.g., "Clinician Follow-Up Interview: Ariel"
          if (is.na(clinician_name) && grepl("Clinician Follow-Up Interview:", summary)) {
            title_clinician <- sub(".*Clinician Follow-Up Interview:\\s*", "", summary)
            title_clinician <- trimws(title_clinician)
            if (nchar(title_clinician) > 0) {
              clinician_name <- title_clinician
            }
          }
          ...
          session_type <- "Screening"  # Default for ABM calendar events
          if (grepl("Clinician Follow-Up Interview", summary)) {
            session_type <- "Follow-Up Interview"
            # For follow-ups, extract week from description (e.g., "Which Session Event?: Wk. 3")
        ...
        data.frame(
          Time = "No events",
          Event = "No participant sessions scheduled",
          Clinician = "",
          Session = "",
...
trad_compliance <- tryCatch({
  ...
  if (abs_username == "" || abs_password == "") {
    data.frame(Message = "ABS credentials not configured") |>
      tinytable::tt()
  } else {
    result <- abmdash::get_trad_compliance_summary()

    if (nrow(result) > 0) {
      display_data <- data.frame(
        "Participant ID" = result$id,
        "Week" = result$current_week,
        "Sessions Completed" = result$total_sessions,
        "Expected Sessions" = result$expected_sessions,
      ...
      behind_rows <- which(result$sessions_behind > 0)
      if (length(behind_rows) > 0) {
        tt_table <- tt_table |>
          tinytable::style_tt(i = behind_rows, background = "#ffebee")
      }
